...
- `BeforeUpdate(Cancel As Integer)`  
  The BeforeUpdate event occurs before changed data in a control or record is updated.
    - `Cancel As Integer` (required): The setting determines if the BeforeUpdate event occurs. Setting the _Cancel_ argument to True (1) cancels the BeforeUpdate event.
- `AfterUpdate()`  
