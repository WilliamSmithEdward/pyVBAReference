...
**Remarks:** The Example section describes the following properties and methods for returning a Chart object: - The Chart property. - The AddChart method.
...
- `Paste([Type As Variant])`  
  Pastes chart data from the Clipboard into the chart.
    - `Type As Variant` (optional): Specifies the chart information to paste if a chart is on the Clipboard. Can be one of the following values: 4104 Everything will be pasted. This is the default value. 4122 Copied source formats are pasted. 4123 Formulas are pasted.
- `Export(FileName As String, [FilterName As Variant], [Interactive As Variant]) As Boolean`  
  Exports the chart in a graphic format.
    ...
    - `Interactive As Variant` (optional): True to display the dialog box that contains the filter-specific options. False to indicate that Word should use the default values for the filter. The default is False.
...
- `FullSeriesCollection([Index As Variant]) As Object`  
  Returns the collection of all the series in the specified chart, or the specified series.
    - `Index As Variant` (optional): The index number in the series collection of the series to return.
