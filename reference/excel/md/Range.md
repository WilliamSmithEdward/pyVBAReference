...
**Remarks:** The default member of Range forwards calls without parameters to the Value property and calls with parameters to the Item member. Accordingly, someRange = someOtherRange is equivalent to someRange.Value = someOtherRange.Value, someRange(1) to someRange.Item(1) and someRange(1,1) to someRange.Item(1,1). The following properties and methods for returning a Range object are described in the Example section: - Range and Cells properties of the Worksheet object - Range and Cells properties of the Range object - Rows and Columns properties of the Worksheet object - Rows and Columns properties of the Range object - Offset property of the Range object - Union method of the Application object
...
- `AutoComplete(String As String) As String`  
  Returns an AutoComplete match from the list. If there's no AutoComplete match or if more than one entry in the list matches the string to complete, this method returns an empty string.
    - `String As String` (required): The string to complete.
- `AutoFill(Destination As Range, [Type As XlAutoFillType]) As Variant`  
  Performs an autofill on the cells in the specified range.
    - `Destination As Range` (required): The cells to be filled. The destination must include the source range.
    - `Type As XlAutoFillType` (optional): Specifies the fill type.
- `AutoFit() As Variant`  
...
- `ColumnDifferences(Comparison As Variant) As Range`  
  Returns a Range object that represents all the cells whose contents are different from the comparison cell in each column.
    - `Comparison As Variant` (required): A single cell to compare to the specified range.
- `Consolidate([Sources As Variant], [Function As Variant], [TopRow As Variant], [LeftColumn As Variant], [CreateLinks As Variant]) As Variant`  
  Consolidates data from multiple ranges on multiple worksheets into a single range on a single worksheet. Variant.
    - `Sources As Variant` (optional): The sources of the consolidation as an array of text reference strings in R1C1-style notation. The references must include the full path of sheets to be consolidated.
    - `Function As Variant` (optional): One of the constants of XlConsolidationFunction, which specifies the type of consolidation.
    - `TopRow As Variant` (optional): True to consolidate data based on column titles in the top row of the consolidation ranges. False to consolidate data by position. The default value is False.
    - `LeftColumn As Variant` (optional): True to consolidate data based on row titles in the left column of the consolidation ranges. False to consolidate data by position. The default value is False.
    - `CreateLinks As Variant` (optional): True to have the consolidation use worksheet links. False to have the consolidation copy the data. The default value is False.
- `Copy([Destination As Variant]) As Variant`  
  Copies the range to the specified range or to the Clipboard.
    - `Destination As Variant` (optional): Specifies the new range to which the specified range will be copied. If this argument is omitted, Microsoft Excel copies the range to the Clipboard.
- `CopyFromRecordset(Data As IUnknown, [MaxRows As Variant], [MaxColumns As Variant]) As Long`  
  Copies the contents of an ADO or DAO Recordset object onto a worksheet, beginning at the upper-left corner of the specified range. If the Recordset object contains fields with OLE objects in them, this method fails.
    - `Data As IUnknown` (required): The Recordset object to copy into the range.
    - `MaxRows As Variant` (optional): The maximum number of records to copy onto the worksheet. If this argument is omitted, all the records in the Recordset object are copied.
    - `MaxColumns As Variant` (optional): The maximum number of fields to copy onto the worksheet. If this argument is omitted, all the fields in the Recordset object are copied.
- `CopyPicture([Appearance As XlPictureAppearance], [Format As XlCopyPictureFormat]) As Variant`  
  Copies the selected object to the Clipboard as a picture. Variant.
    - `Appearance As XlPictureAppearance` (optional): Specifies how the picture should be copied.
    - `Format As XlCopyPictureFormat` (optional): The format of the picture.
- `CreateNames([Top As Variant], [Left As Variant], [Bottom As Variant], [Right As Variant]) As Variant`  
  Creates names in the specified range, based on text labels in the sheet.
    - `Top As Variant` (optional): True to create names by using labels in the top row. The default value is False.
    - `Left As Variant` (optional): True to create names by using labels in the left column. The default value is False.
    - `Bottom As Variant` (optional): True to create names by using labels in the bottom row. The default value is False.
    - `Right As Variant` (optional): True to create names by using labels in the right column. The default value is False.
- `Cut([Destination As Variant]) As Variant`  
  Cuts the object to the Clipboard or pastes it into a specified destination.
    - `Destination As Variant` (optional): The range where the object should be pasted. If this argument is omitted, the object is cut to the Clipboard.
...
- `Delete([Shift As Variant]) As Variant`  
  Deletes the object.
    - `Shift As Variant` (optional): Used only with Range objects. Specifies how to shift cells to replace deleted cells. Can be one of the following XlDeleteShiftDirection constants: xlShiftToLeft or xlShiftUp. If this argument is omitted, Microsoft Excel decides based on the shape of the range.
...
- `EditionOptions(Type As XlEditionType, Option As XlEditionOptionsOption, [Name As Variant], [Reference As Variant], [Appearance As XlPictureAppearance], [ChartSize As XlPictureAppearance], [Format As Variant]) As Variant`  
  You have requested Help for a Visual Basic keyword used only on the Macintosh. For information about this keyword, consult the language reference Help included with Microsoft Office Macintosh Edition.
    - `Type As XlEditionType` (required): See the Help included with Microsoft Office Macintosh Edition.
    - `Option As XlEditionOptionsOption` (required): See the Help included with Microsoft Office Macintosh Edition.
    - `Name As Variant` (optional): See the Help included with Microsoft Office Macintosh Edition.
    - `Reference As Variant` (optional): See the Help included with Microsoft Office Macintosh Edition.
    - `Appearance As XlPictureAppearance` (optional): See the Help included with Microsoft Office Macintosh Edition.
    - `ChartSize As XlPictureAppearance` (optional): See the Help included with Microsoft Office Macintosh Edition.
    - `Format As Variant` (optional): See the Help included with Microsoft Office Macintosh Edition.
- `FillDown() As Variant`  
...
- `Find(What As Variant, [After As Variant], [LookIn As Variant], [LookAt As Variant], [SearchOrder As Variant], [SearchDirection As XlSearchDirection], [MatchCase As Variant], [MatchByte As Variant], [SearchFormat As Variant]) As Range`  
  Finds specific information in a range.
    ...
    - `SearchDirection As XlSearchDirection` (optional): Can be one of the following XlSearchDirection constants: xlNext or xlPrevious.
    - `MatchCase As Variant` (optional): True to make the search case-sensitive. The default value is False.
    - `MatchByte As Variant` (optional): Used only if you have selected or installed double-byte language support. True to have double-byte characters match only double-byte characters. False to have double-byte characters match their single-byte equivalents.
...
- `InsertIndent(InsertAmount As Long)`  
  Adds an indent to the specified range.
    - `InsertAmount As Long` (required): The amount to be added to the current indent.
- `Insert([Shift As Variant], [CopyOrigin As Variant]) As Variant`  
  Inserts a cell or a range of cells into the worksheet or macro sheet and shifts other cells away to make space.
    - `Shift As Variant` (optional): Specifies which way to shift the cells. Can be one of the following XlInsertShiftDirection constants: xlShiftToRight or xlShiftDown. If this argument is omitted, Microsoft Excel decides based on the shape of the range.
    - `CopyOrigin As Variant` (optional): The copy origin; that is, from where to copy the format for inserted cells. Can be one of the following XlInsertFormatOrigin constants: xlFormatFromLeftOrAbove (default) or xlFormatFromRightOrBelow.
- `Justify() As Variant`  
...
- `Merge([Across As Variant])`  
  Creates a merged cell from the specified Range object.
    - `Across As Variant` (optional): True to merge cells in each row of the specified range as separate merged cells. The default value is False.
- `UnMerge()`  
...
- `NavigateArrow([TowardPrecedent As Variant], [ArrowNumber As Variant], [LinkNumber As Variant]) As Variant`  
  Navigates a tracer arrow for the specified range to the precedent, dependent, or error-causing cell or cells. Selects the precedent, dependent, or error cells and returns a Range object that represents the new selection. This method causes an error if it's applied to a cell without visible tracer arrows.
    - `TowardPrecedent As Variant` (optional): Specifies the direction to navigate; True to navigate toward precedents, False to navigate toward dependents.
    - `ArrowNumber As Variant` (optional): Specifies the arrow number to navigate; corresponds to the numbered reference in the cell's formula.
...
- `Parse([ParseLine As Variant], [Destination As Variant]) As Variant`  
  Parses a range of data and breaks it into multiple cells. Distributes the contents of the range to fill several adjacent columns; the range can be no more than one column wide.
    ...
    - `Destination As Variant` (optional): A Range object that represents the upper-left corner of the destination range for the parsed data. If this argument is omitted, Microsoft Excel parses in place.
...
- `RowDifferences(Comparison As Variant) As Range`  
  Returns a Range object that represents all the cells whose contents are different from those of the comparison cell in each row.
    - `Comparison As Variant` (required): A single cell to compare with the specified range.
...
- `ShowDependents([Remove As Variant]) As Variant`  
  Draws tracer arrows to the direct dependents of the range.
    - `Remove As Variant` (optional): True to remove one level of tracer arrows to direct dependents. False to expand one level of tracer arrows. The default value is False.
...
- `ShowPrecedents([Remove As Variant]) As Variant`  
  Draws tracer arrows to the direct precedents of the range.
    - `Remove As Variant` (optional): True to remove one level of tracer arrows to direct precedents. False to expand one level of tracer arrows. The default value is False.
...
- `SubscribeTo(Edition As String, [Format As XlSubscribeToFormat]) As Variant`  
  You have requested Help for a Visual Basic keyword used only on the Macintosh. For information about this keyword, consult the language reference Help included with Microsoft Office Macintosh Edition.
    ...
    - `Format As XlSubscribeToFormat` (optional): See help included with Microsoft Office Macintosh Edition.
- `Subtotal(GroupBy As Long, Function As XlConsolidationFunction, TotalList As Variant, [Replace As Variant], [PageBreaks As Variant], [SummaryBelowData As XlSummaryRow]) As Variant`  
  Creates subtotals for the range (or the current region, if the range is a single cell).
    - `GroupBy As Long` (required): The field to group by, as a one-based integer offset. For more information, see the example.
    - `Function As XlConsolidationFunction` (required): The subtotal function.
    - `TotalList As Variant` (required): An array of 1-based field offsets, indicating the fields to which the subtotals are added. For more information, see the example.
    - `Replace As Variant` (optional): True to replace existing subtotals. The default value is True.
    - `PageBreaks As Variant` (optional): True to add page breaks after each group. The default value is False.
    - `SummaryBelowData As XlSummaryRow` (optional): Places the summary data relative to the subtotal.
- `Table([RowInput As Variant], [ColumnInput As Variant]) As Variant`  
  Creates a data table based on input values and formulas that you define on a worksheet.
    - `RowInput As Variant` (optional): A single cell to use as the row input for your table.
    - `ColumnInput As Variant` (optional): A single cell to use as the column input for your table.
- `TextToColumns([Destination As Variant], [DataType As XlTextParsingType], [TextQualifier As XlTextQualifier], [ConsecutiveDelimiter As Variant], [Tab As Variant], [Semicolon As Variant], [Comma As Variant], [Space As Variant], [Other As Variant], [OtherChar As Variant], [FieldInfo As Variant], [DecimalSeparator As Variant], [ThousandsSeparator As Variant], [TrailingMinusNumbers As Variant]) As Variant`  
  Parses a column of cells that contain text into several columns.
    - `Destination As Variant` (optional): A Range object that specifies where Microsoft Excel will place the results. If the range is larger than a single cell, the top left cell is used.
...
- `AddComment([Text As Variant]) As Comment`  
  Adds a comment to the range.
    - `Text As Variant` (optional): The comment text.
- `ClearComments()`  
...
- `Speak([SpeakDirection As Variant], [SpeakFormulas As Variant])`  
  Causes the cells of the range to be spoken in row order or column order.
    - `SpeakDirection As Variant` (optional): The speak direction, by rows or columns.
    - `SpeakFormulas As Variant` (optional): True will cause formulas to be sent to the Text-To-Speech (TTS) engine for cells that have formulas. The value is sent if the cells don't have formulas. False (default) will cause values to always be sent to the TTS engine.
- `PasteSpecial([Paste As XlPasteType], [Operation As XlPasteSpecialOperation], [SkipBlanks As Variant], [Transpose As Variant]) As Variant`  
  Pastes a Range object that has been copied into the specified range.
    - `Paste As XlPasteType` (optional): The part of the range to be pasted, such as xlPasteAll or xlPasteValues.
    - `Operation As XlPasteSpecialOperation` (optional): The paste operation, such as xlPasteSpecialOperationAdd.
    - `SkipBlanks As Variant` (optional): True to have blank cells in the range on the clipboard not be pasted into the destination range. The default value is False.
    - `Transpose As Variant` (optional): True to transpose rows and columns when the range is pasted. The default value is False.
- `RemoveDuplicates([Columns As Variant], [Header As XlYesNoGuess])`  
  Removes duplicate values from a range of values.
    - `Columns As Variant` (optional): Array of indexes of the columns that contain the duplicate information.
...
- `BorderAround([LineStyle As Variant], [Weight As XlBorderWeight], [ColorIndex As XlColorIndex], [Color As Variant], [ThemeColor As Variant]) As Variant`  
  Adds a border to a range and sets the Color, LineStyle, and Weight properties of the Border object for the new border. Variant.
    - `LineStyle As Variant` (optional): One of the constants of XlLineStyle specifying the line style for the border.
    - `Weight As XlBorderWeight` (optional): The border weight.
    - `ColorIndex As XlColorIndex` (optional): The border color, as an index into the current color palette or as an XlColorIndex constant.
    - `Color As Variant` (optional): The border color, as an RGB value.
    - `ThemeColor As Variant` (optional): The theme color, as an index into the current color theme or as an XlThemeColor value.
- `AllocateChanges()`  
...
- `AddCommentThreaded(Text As String) As CommentThreaded`  
  Adds a new modern threaded comment to the range if no comment already exists.
    - `Text As String` (required): The comment text.
...
- `ConvertToLinkedDataType(ServiceID As Long, LanguageCulture As String)`  
  Attempts to convert all the cells in the range to a Linked data type such as Stocks or Geography.
    - `ServiceID As Long` (required): The ID of the service that will provide the linked entity.
    - `LanguageCulture As String` (required): A string representing the LCID of the language and culture that you would like to use for the linked entity.
...
- `Replace(What As Variant, Replacement As Variant, [LookAt As Variant], [SearchOrder As Variant], [MatchCase As Variant], [MatchByte As Variant], [SearchFormat As Variant], [ReplaceFormat As Variant], [FormulaVersion As Variant]) As Boolean`  
  Returns a Boolean indicating characters in cells within the specified range. Using this method doesn't change either the selection or the active cell.
    - `What As Variant` (required): The string that you want Microsoft Excel to search for.
    - `Replacement As Variant` (required): The replacement string.
    - `LookAt As Variant` (optional): Can be one of the following XlLookAt constants: xlWhole or xlPart.
    - `SearchOrder As Variant` (optional): Can be one of the following XlSearchOrder constants: xlByRows or xlByColumns.
    - `MatchCase As Variant` (optional): True to make the search case-sensitive.
    - `MatchByte As Variant` (optional): Use this argument only if you have selected or installed double-byte language support in Microsoft Excel. True to have double-byte characters match only double-byte characters. False to have double-byte characters match their single-byte equivalents.
    - `SearchFormat As Variant` (optional): The search format for the method.
    - `ReplaceFormat As Variant` (optional): The replace format for the method.
    - `FormulaVersion As Variant` (optional): Determines whether to replace in Dynamic Array formula state constants: xlReplaceFormula or xlReplaceFormula2.
